...
export function StructuredNote({ note, noteNumber, procedures, hasSupplyList, onSupplyClick, isHighlighted, onAction }: StructuredNoteProps) {
  ...
  return (
    <Card className={cn(
      ...
      isHighlighted && 'ring-2 ring-accent border-accent'
    )}>
      <CardHeader className="flex flex-row items-center justify-between px-5 py-3 border-b border-border space-y-0">
        <div>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className="text-xs text-muted">
              {note.nurse_name} | <time dateTime={note.created_at} className="font-mono" suppressHydrationWarning>{formatNoteTimestamp(note.created_at)}</time> | {note.shift} shift
            </span>
        ...
        <div className="flex items-center gap-2">
          {hasSupplyList && onSupplyClick && (
            <Button
              ...
              className="gap-1 text-xs font-medium rounded-full bg-accent/10 text-accent hover:bg-accent/20"
            >
              <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
      ...
      {note.flagged && (
        <div className={cn(
          ...
          flagType === 'critical'
            ? 'bg-flag-critical-bg border border-flag-critical/20'
            : 'bg-flag-warning-bg border border-flag-warning/20'
        )} role="alert">
          <div className="flex items-start gap-2">
            <svg className={cn('w-4 h-4 mt-0.5 shrink-0', flagType === 'critical' ? 'text-flag-critical' : 'text-flag-warning')} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
            </svg>
            <p className={cn('text-sm', flagType === 'critical' ? 'text-flag-critical' : 'text-flag-warning')}>
              {note.flag_reason}
            </p>
          </div>
        </div>
        ...
        {soapSections.map((section) => {
          ...
          return (
            <div key={section.key} className={cn(
              section.nested && 'pl-3 border-l-2 border-border'
            )}>
              <span className={cn(
                'font-semibold uppercase tracking-widest text-xs text-secondary'
              )}>
                {section.label}
              </span>
              <p className={cn('text-sm text-primary mt-1', section.nested ? 'leading-relaxed' : 'leading-[1.75]')}>
                {section.content || '\u2014'}
              </p>
            </div>
          )
